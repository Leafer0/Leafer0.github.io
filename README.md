...
配色集中在 `src/input.css` 顶部的 CSS 变量里，深色模式在 `.dark` 下覆盖：

| 变量 | 浅色 | 深色 | 用途 |
| --- | --- | --- | --- |
| `--garden-ink` | `#2f3a33` | `#e6ece7` | 正文主色（偏绿的深墨） |
| `--garden-leaf` | `#6f9a76` | `#7fa987` | 叶片 |
| `--garden-gold` | `#c8a86b` | `#d4b87e` | 细金边框与花心 |
| `--garden-paper` | `#fbfaf6` | 半透明深色 | 正文卡片底色 |
| `--garden-line` | 淡藤线 | 淡藤线 | 内层边框 |

想整体换风格，改这几个变量即可，不必逐条找颜色。

### 草木纹样（内联 SVG）

纹样定义在 `tools/build-botanical.js`，生成到 `botanical.js`：

```bash
node tools/build-botanical.js    # 重新生成（含路径越界校验）
```

六个纹样：`vine`（藤蔓）、`blossom`（花）、`sprig`（细枝）、
`fern`（蕨叶）、`divider`（花饰分隔线）、`cornerCluster`（角落大簇）。

**为什么用 SVG 而不是贴图**：任意缩放都清晰、体积只有几 KB、
颜色跟随 CSS 变量（深色模式不用另做一套图）、并且能做"描边生长"动画。

用法：`v-html="botanical('vine')"`；
入场动画需要生长效果时传 `{ grow: true, delay: 0.15 }`。

### 正文装帧

内容外层是 `.garden-frame`：半透明纸面 + 内层藤线 + 外层细金线（用
`outline` + `outline-offset` 做出"双线装帧"的感觉），四角挂 `.garden-corner`，
宽屏时两侧还有 `.garden-vine`。
...
| 0.00s | 纸面淡入 |
| 0.15s | 四角藤蔓开始**描边生长** |
| 0.75s | 叶片与花朵**依次绽放**（每片间隔 0.08s） |
| 1.15s | 标题流光出现，下方金线生长 |
| 1.70s | 帷幕模糊淡出 |
| 2.50s | 移除遮罩 |

**三种进入方式**（见 `startEverything(mode)`）：

- `enter` —— 正常点击：播完帷幕淡出，音乐起播
- `skip` —— 点「跳过动画」：**立即**移除，不做过渡
  （读者已表达"别放了"，再播 0.75 秒淡出是违背意愿的）
- `auto` —— 直接移除但**不主动播放音乐**：浏览器不允许非用户手势触发自动播放

### 两个必须记住的注意点

**1. 改完模板要重建 CSS。**
已经踩过两次：加「跳过动画」按钮后没跑 `npm run build:css`，
`bottom-7` / `right-7` 两个类没生成，按钮落到静态位置、正好压在标题上。
**任何模板类名改动之后都要重建。**

**2. 系统开启「减少动态效果」时动画会被压掉。**
`src/input.css` 里有 `@media (prefers-reduced-motion: reduce)`
把动画时长压到 0.01ms —— 这是无障碍特性，要保留。
但在这类机器上用截图工具会拍到"每帧都是终态"，
看上去像动画坏了。`tools/shot-intro.js` 与 `tools/measure-intro-anim.js`
已显式覆盖该偏好，仅为采集动画过程。
